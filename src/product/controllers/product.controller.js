...
export const getAllProducts = async (req, res, next) => {
  ...
  try {
    const {page,keyword,category,pricegte,pricelte,ratinggte,ratinglte} = req.query;
    console.log(page,keyword,category,pricegte,pricelte,ratinggte,ratinglte)
    const products= await getAllProductsRepo(page,keyword,category,pricegte,pricelte,ratinggte,ratinglte);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    if(reviews[isReviewExistIndex].user.toString() === req.user._id.toString()){
      ...
      next(new ErrorHandler(400,'unauthorized to dalete the Review!'))
    }
